"""
Guided xdevs.py Code Generation Pipeline.

Approach: opencode with structural guidance prompt, forcing xdevs.py framework.
- Tells opencode to plan component hierarchy and interface contracts first
- Then implement bottom-up using xdevs.py (Atomic/Coupled models)
- Still uses opencode's agent loop (has edit loop risk, but better guided than bare)

This is the xdevs counterpart to run_guided_pipeline.py.
"""
...
GUIDED_XDEVS_PROMPT = """You are a DEVS simulation expert. Build a complete Python simulation system using **xdevs.py** (formal DEVS framework).
...
- How do they connect (Coupled model with EIC/IC/EOC couplings)?
...
1. Create a `<SystemName>_libs/` directory for sub-models (or put everything in run.py)
2. Write each Atomic model with proper state machine methods
3. Write the top-level Coupled model that composes sub-models
4. Write `run.py` as the entry point with Coordinator
...
class ModelName(Atomic):
    ...
    def exit(self):
        # Cleanup and final KPI logging. MUST be implemented.
        pass
```
...
2. **Use `Coordinator`** for running simulations with Coupled models, NOT `Simulator`.
...
6. **Coupled models**: ONLY `__init__` — no `deltint`, `deltext`, `lambdaf`, `initialize`.
7. **add_component** BEFORE **add_coupling**.
8. **JSONL output**: `print(json.dumps({"time": ..., "entity": "...", "event": "...", "payload": {...}}), file=sys.stdout, flush=True)`
...
def run_guided_xdevs(config_path: str, workspace: str, model_id: str) -> dict:
    ...
    prompt = GUIDED_XDEVS_PROMPT.format(
        requirements=requirements_text,
        workspace=str(workspace_path),
    )

    # Write prompt to a file so opencode can read it
    prompt_file = workspace_path / "_guided_xdevs_prompt.txt"
    prompt_file.write_text(prompt, encoding="utf-8")

    cmd = [
        "opencode",
        "run",
        f"Read the requirements from _guided_xdevs_prompt.txt and implement the DEVS simulation system using xdevs.py. Follow all instructions carefully.",
    ]

    print(f"[Guided-xdevs] Running opencode with structural guidance...")
...
def main():
    parser = argparse.ArgumentParser(description="Guided xdevs.py Code Generation Pipeline")
    ...
    result = run_guided_xdevs(args.config, args.workspace, args.model_id)
    print("\n<<<GENERATION_RESULT>>>")
